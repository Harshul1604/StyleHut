...
def shop(Request,mc,br,sc):
    
    if(mc=='all' and br=='all' and sc=='all'):
        
        data = Product.objects.all()
    elif(mc=='all' and br=='all' and sc!='all'):
    ...
    subcategory=Subcategory.objects.all()
    maincategory= Maincategory.objects.all()
    return render(Request,"shop.html",{'data':data,'maincategory':maincategory,'subcategory':subcategory,'brand':brand,'mc':mc,'br':br,'sc':sc,'count':count})

def shopdetail(Request,num):
...
def addToCartPage(Request,num):
    ...
    if(cart):
        if(str(p.id) in cart):
            ...
            cart.setdefault(str(p.id),{'pid':p.id,'name':p.name,'color':p.color,'size':p.size,'price':p.finalprice,'qty':1,'total':p.finalprice,'pic':p.pic1.url})
    else:
        cart = {str(p.id):{'pid':p.id,'name':p.name,'color':p.color,'size':p.size,'price':p.finalprice,'qty':1,'total':p.finalprice,'pic':p.pic1.url}}
...
def orderPage(Request,num):
    if(Request.method=="POST"):
        ...
        if(cart is None):
            ...
            subject = 'Your Order has been Placed : Team StyleHut'
            message =   "Thanks to Shop With Us\nYour order Has has been placed\nNow You Can Track Your Orders in Profile Page\nTeam Stylehut"
            email_from = settings.EMAIL_HOST_USER
